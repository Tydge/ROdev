...
- 五个实例均由 `world_ai` 按实时等级、职业、攻防/MATK、技能、怪物强度、同图风险、元素克制和免费路线选择练级目标；只用原生刷怪，不修改地图刷怪。
- 评分层排除 MVP/Boss、明显越级和 kill-cost proxy 超标（物理 20、魔法 24）的目标；Mage / Acolyte 用 MATK（Fire Bolt / Holy Light）而非物攻选怪。实战中长时间零击杀或反复死亡时，会将该怪物@地图冷却 30 分钟后自动重新选择。
- 执行路线默认最多 6 跳（`world_ai_exec_max_hops 6`，可配 1..10），收费/NPC/特殊传送仍保持关闭。
- 普攻、自动拾取、小范围巡逻均已开启。
